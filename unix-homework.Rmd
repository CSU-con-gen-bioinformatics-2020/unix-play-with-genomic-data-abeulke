...
All these files that start with . are hidden files
```
...
```
ls f (5 strokes with tab)
then i get: ls fastq/
tab again (6) and I get ls fastq/DPCH_plate1_
add A06 +tab (now I'm at 10) and get ls fastq/DPCH_plate1_A06_S6.R
add 1 + tab (at 12) and now I get the full file name

fewest keystrokes: 12
...
cd fastq
ls DPCh_plate1_[A-Z]*_S*.R1.fq.gz
cp DPCh_plate1_[A-Z]*_S*.R1.fq.gz ../lonely-fastqs/R1
cp DPCh_plate1_[A-Z]*_S*.R2.fq.gz ../lonely-fastqs/R2
```
...
echo "Whoa! No spaces, please!" > bad\ directory\ name\ with\ spaces/'bad filename with spaces'
...
```{bash}
cat bad\ directory\ name\ with\ spaces/bad\ filename\ with\ spaces
```
...
```
Using the 'ls' command before removing anything is a good way to make sure your code
will only select the files or directories that you want to delete!
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
It may have deleted directories or files with similar names
...
```{bash}
mkdir A05_S5
cp {bam,sam,fastq}/DPCh_plate1_A05* A05_S5
...
```
The read names are the same in R1 and R2. The differ at the very end where it is either
/1 (R1) or /2 (R2 file)
```
...
```{bash}
gzcat fastq/DPCh_plate1_[A-H]*_S*.R1.fq.gz | wc 
gzcat fastq/DPCh_plate1_[A-H]*_S*.R2.fq.gz | wc
```
...
#From Eric's Email: Try giving a 'chmod 664 count_files.sh" and then try the above command of executing the file
```{bash}
